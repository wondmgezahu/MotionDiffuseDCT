...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        # md=model(x, self._scale_timesteps(t), **model_kwargs)
        # breakpoint()
        # from calflops import calculate_flops
        # input_shape = (128,20)  # define your input shape here
        # flops, macs, params = calculate_flops(model=md,input_shape= input_shape,include_backPropagation=True)
        # print("FLOPs:%s  MACs:%s  Params:%s \n" %(flops, macs, params))
        # breakpoint()
        # from thop import profile

        # # Assuming 'model' is your PyTorch model and 'input' is your input tensor
        # input_=(x, self._scale_timesteps(t), model_kwargs)
    ...
    def p_sample_loop_progressive(
        ...
        n_pre=40
        import torch
        img=torch.matmul(model_kwargs['noise_dct'][:n_pre], img).to(img.device)
        # breakpoint()
        # img = torch.matmul(model_kwargs['noise_dct'], img).to(img.device)
        for i in indices:
    ...
    def ddim_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            img = th.randn(*shape, device=device)
        self.num_timesteps=50 # added for controllability
        indices = list(range(self.num_timesteps))[::-1]
